package com.xxx.market.web.core.security;
...
import com.xxx.market.web.core.plugin.shiro.SubjectKit;
...
public class ShiroAuthorizingRealm extends AuthorizingRealm{
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		admin = SellerUser.dao.findFirst("select * from t_seller_user where phone = ? ", username);
...
//		List<Role> roles = null;
		SellerUser admin = SellerUser.dao.findFirst("select * from t_seller_user where phone = ? ", loginName);
		
		if(admin == null) SubjectKit.getSubject().logout();
		/*if (admin != null) {
			// 遍历角色
