...
public abstract class BaseMobileController extends ApiController {
	...
	protected Long getSellerId(){
		return getBuyerUser().getSellerId();
	}
	...
	protected AuthUser getAuthUser(HttpServletRequest request){
		if(PropKit.getBoolean("jfinal.devmode")){
			return AuthUser.dao.findFirst("select * from " + AuthUser.table + " where app_id=? ", WxSdkPropKit.get("wx_app_id"));
		}
		...
		if(authUser == null){
			authUser = AuthUser.dao.findFirst("select * from " + AuthUser.table + " where app_id=? and active=1 ", WxSdkPropKit.get("wx_app_id"));
