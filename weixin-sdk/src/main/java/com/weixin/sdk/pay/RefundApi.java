 ...
 * 查询出需要退款的用户
 * select 
T1.nickname, T1.headimgurl,
T2.duobao_rc, T2.transaction_id
from t_user T1,
(
select tdr.item_id, tdr.user_id, tdr.transaction_id, tdr.duobao_rc 
from t_duobao_record tdr 
where tdr.qishu_id in(select tdq.id from t_duobao_qishu tdq where tdq.`status` !=3)
